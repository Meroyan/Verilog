...
//  output             inst_jbr,    // 指令为跳转分支指令,五级流水不需要
    ...
    input              EXE_over,    //（新增）EXE完成
    input              MEM_over,    //（新增）MEM完成
    input      [ 31:0] EXE_value,   //（新增）EXE的计算结果
    input      [ 31:0] MEM_value,   //（新增）MEM的结果
    input              EXE_bypass,  //（新增）是否启用EXE旁路
    input              MEM_bypass,  //（新增）是否启用MEM旁路
    //展示PC
    output     [ 31:0] ID_pc,
    output     congestion           //（新增）流水线中是否存在冒险
);
//-----{IF->ID总线}begin
    wire [31:0] pc;
    wire [31:0] inst;
...
//-----{IF->ID总线}end
    ...
    wire [31:0] real_rs_value;  //（新增）
    wire [31:0] real_rt_value;  //（新增）
...
//-----{指令译码}begin
    ...
    //（新增）断点指令
    wire inst_BRK;
    assign inst_BRK = (op == 6'b001110) & (funct == 6'b000000);
    
    wire break;
    assign break = inst_BRK;
    
    //（新增）非法指令
    wire all_ins = inst_ADDU | inst_SUBU | inst_SLT | inst_AND |
                   ...
                   inst_SYSCALL | inst_BRK;
    wire error_ins;
    assign error_ins = ~all_ins;
...
assign j_target = inst_jr ? real_rs_value : {bd_pc[31:28],target,2'b00};
    //branch指令
    wire rs_equql_rt;
    wire rs_ez;
    wire rs_ltz;
...
//    assign rs_ltz      = rs_value[31];            // rs寄存器值小于0
    assign rs_equql_rt = (real_rs_value == real_rt_value);
    assign rs_ez       = ~(|real_rs_value);     // rs寄存器值为0
    assign rs_ltz      = real_rs_value[31];     // rs寄存器值小于0
...
//（新增）
wire inst_rs_is_rdest;      //rs是否是目标寄存器
wire inst_rt_is_rdest;      //rt是否是目标寄存器
...
//    (~inst_jbr | IF_over)即是(~inst_jbr | (inst_jbr & IF_over))
    ...
    assign real_rs_value = (rs_wait & ((rs == EXE_wdest) & EXE_bypass & EXE_over)) ? EXE_value : 
                          (rs_wait & ((rs == MEM_wdest) & MEM_bypass & MEM_over)) ? MEM_value :
                          rs_value;
    assign real_rt_value = (rt_wait & ((rt == EXE_wdest) & EXE_bypass & EXE_over)) ? EXE_value :
                          (rt_wait & ((rt == MEM_wdest) & MEM_bypass & MEM_over)) ? MEM_value :
                          rt_value;
                      
    assign congestion=rs_wait |rt_wait|inst_load;
    
//    assign ID_over = ID_valid & ~rs_wait & ~rt_wait & (~inst_jbr | IF_over);
    assign ID_over = ID_valid  & (~inst_jbr | IF_over) 
                         & (~rs_wait | ((rs == EXE_wdest) & EXE_bypass & EXE_over) | ((rs == MEM_wdest) & MEM_bypass & MEM_over) ) 
                         & (~rt_wait | ((rt == EXE_wdest) & EXE_bypass & EXE_over) | ((rt == MEM_wdest) & MEM_bypass & MEM_over) );
...
assign alu_operand1 = inst_j_link ? pc : 
    inst_shf_sa ? {27'd0,sa} : real_rs_value;
assign alu_operand2 = inst_j_link ? 32'd8 :  
                      inst_imm_zero ? {16'd0, imm} :
    inst_imm_sign ?  {{16{imm[15]}}, imm} : real_rt_value;
...
assign store_data = real_rt_value;
wire data_related;      //（新增）
assign data_related = (alu_control!=13'b0 ) | (inst_MULT & (!inst_load)) ? 1'b1 : 1'b0;
assign ID_EXE_bus = {multiply,mthi,mtlo,                   //EXE需用的信息,新增
                     alu_control,alu_operand1,alu_operand2,//EXE需用的信息
                     mem_control,store_data,               //MEM需用的信号
                     mfhi,mflo,                            //WB需用的信号,新增
