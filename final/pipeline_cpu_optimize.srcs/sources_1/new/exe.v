...
module exe(                         // 执行级
    ...
    output  reg           EXE_over,    // EXE模块执行完成
    output     [155:0] EXE_MEM_bus, // EXE->MEM总线
     //5级流水新增
    input             clk,       // 时钟
    output     [  4:0] EXE_wdest,   // EXE级要写回寄存器堆的目标地址号
    output     [ 31:0] EXE_value,   //（新增）EXE的计算结果
    output             EXE_bypass,  //（新增）EXE旁路
    //展示PC
    output     [ 31:0] EXE_pc
);
...
//    assign EXE_over = EXE_valid & (~multiply | mult_end);
    ...
    begin
        if(timectrl==1'd0)
        begin
            timectrl=timectrl+1'd1;
        end
        else
        begin
            timectrl=1'd0;
            EXE_over <= EXE_valid & (~multiply | mult_end);
        end
    end
//-----{EXE执行完成}end
...
//-----{EXE->MEM总线}begin
    ...
    assign EXE_value = exe_result;      //（新增）
    assign EXE_MEM_bus = {mem_control,store_data,          //load/store信息和store数据
                          ...
                          data_related,
                          pc};                             //PC
    assign EXE_bypass = data_related;       //（新增）
//-----{EXE->MEM总线}end
//-----{展示EXE模块的PC值}begin
    assign EXE_pc = pc;
//-----{展示EXE模块的PC值}end
