...
module alu(
    ...
    wire alu_add;   //加法操作0
    wire alu_sub;   //减法操作1
    wire alu_slt;   //有符号比较，小于置位，复用加法器做减法2
    wire alu_sltu;  //无符号比较，小于置位，复用加法器做减法3
    wire alu_and;   //按位与4
    wire alu_nor;   //按位或非5
    wire alu_or;    //按位或6
    wire alu_xor;   //按位异或7
    ...
    wire alu_sra;   //算术右移10
    wire alu_lui;   //高位加载11
    //额外添加，比较运算，位运算，数据加载运算
    wire alu_smtu;  //无符号比较，大于等于则置位，复用加法器做减法12
    wire alu_nand;  //按位与非13
    wire alu_mui;   //低位加载14
    
    assign alu_add  = (alu_control==4'b0000) ? 1 : 0;
    assign alu_sub  = (alu_control==4'b0001) ? 1 : 0;
    assign alu_slt  = (alu_control==4'b0010) ? 1 : 0;
    assign alu_sltu = (alu_control==4'b0011) ? 1 : 0;
    assign alu_and  = (alu_control==4'b0100) ? 1 : 0;
    assign alu_nor  = (alu_control==4'b0101) ? 1 : 0;
    assign alu_or   = (alu_control==4'b0110) ? 1 : 0;
    assign alu_xor  = (alu_control==4'b0111) ? 1 : 0;
    assign alu_sll  = (alu_control==4'b1000) ? 1 : 0;
    assign alu_srl  = (alu_control==4'b1001) ? 1 : 0;
    assign alu_sra  = (alu_control==4'b1010) ? 1 : 0;
    assign alu_lui  = (alu_control==4'b1011) ? 1 : 0;
    assign alu_smtu  = (alu_control==4'b1100) ? 1 : 0;
    assign alu_nand  = (alu_control==4'b1101) ? 1 : 0;
    assign alu_mui  = (alu_control==4'b1110) ? 1 : 0;
    ...
    wire [31:0] smtu_result;
    wire [31:0] nand_result;
    wire [31:0] mui_result;
    ...
    assign nand_result = ~and_result;               // （新增）与非结果为与结果按位取反
    assign mui_result = {16'd0, alu_src2[31:16]};  // （新增）低位加载结果为立即数移位至低半字节
    assign lui_result = {alu_src2[15:0], 16'd0};  // 立即数装载结果为立即数移位至高半字节

//-----{加法器}begin
//add,sub,slt(有符号比较),sltu(无符号比较)均使用该模块
    ...
    assign smtu_result = (adder_cout | (adder_result == 32'h00000000)) ? 1'b1 : 1'b0;
