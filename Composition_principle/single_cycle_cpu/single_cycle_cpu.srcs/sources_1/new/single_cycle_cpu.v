...
//---------------------------------{译码}begin------------------------------------//
    ...
    wire inst_SMTU, inst_NAND , inst_MUI; 
    assign inst_SMTU = op_zero & sa_zero    & (funct == 6'b110000);// 大于等于则置位
    assign inst_NAND   = op_zero & sa_zero    & (funct == 6'b110001);// 逻辑与非运算
    assign inst_MUI   = (op == 6'b110010);                  // 立即数装载低半字节
    ...
    wire inst_smtu, inst_nand, inst_mui;
    assign inst_smtu = inst_SMTU;   //大于等于则置位
    assign inst_nand = inst_NAND;   //按位与非
    assign inst_mui  = inst_MUI;    //立即数装载低位
...
//    assign inst_imm_sign = inst_ADDIU | inst_LUI | inst_LW | inst_SW ;
    assign inst_imm_sign = inst_ADDIU | inst_LUI | inst_LW | inst_SW | inst_MUI;
...
//---------------------------------{写回}begin------------------------------------//
    ...
    assign inst_wdest_rt = inst_ADDIU | inst_LW | inst_LUI | inst_MUI;
