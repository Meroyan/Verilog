...
//    end
    ...
    begin
        ...
        begin
            IF_over <= 1'b0;
            wait_cycle <= 1'b0; // 重置等待周期
        end
        else if (wait_cycle) 
        begin
            IF_over <= IF_valid; // 在等待周期后执行取指有效信号
            wait_cycle <= 1'b0; // 清除等待标志
        end
        else 
        begin
            wait_cycle <= 1'b1; // 设置等待周期
            IF_over <= 1'b0; // 仍然保持IF_over为0
        end
    end
