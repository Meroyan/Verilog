...
module adder_display(
    ...
    //拨码开关，用于选择输入数
    //00:输入为加数1(add_operand1);01:输入为加数2(add_operand2)
    //10:输入为加数3(add_operand1);11:输入为加数4(add_operand2)
    input input_sel1, 
    input input_sel2,
...
//-----{调用加法模块}begin
    ...
    assign adder_cin[1] = sw_cin1;
    assign adder_cin[0] = sw_cin2;
...
//建议对每一个数的输入，编写单独一个always块
    //当input_sel1为0时，input_sel2为0时,表示输入数为加数1，即operand1
    always @(posedge clk)
    begin
        if (!resetn)
        begin
    ...
    //当input_sel1为0时，input_sel2为1时,表示输入数为加数2，即operand2
    always @(posedge clk)
    begin
        if (!resetn)
        begin
        ...
        //当input_sel1为1时，input_sel2为0时,表示输入数为加数3，即operand3
    always @(posedge clk)
    begin
        if (!resetn)
        begin
        ...
        //当input_sel1为1时，input_sel2为1时,表示输入数为加数4，即operand4
    always @(posedge clk)
    begin
        if (!resetn)
        begin
